...
# Read 'all_drones.csv'
all_paths_df = pd.read_csv('ESC_not_drones.csv', header=None)
...
for index, row in all_paths_df.iterrows():
    ...
    labels_df = pd.DataFrame([['not_drone']])
    ...
    # Print confirmation
    print(f"'label.csv' created at {label_csv_path} with 'not_drone' in [0][0]")
